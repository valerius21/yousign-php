...
use OpenAPI\Server\Api\IdentityVideoVerificationApiInterface;

class IdentityVideoVerificationController extends Controller
{
    /**
     * Constructor
     */
    public function __construct(
        private readonly IdentityVideoVerificationApiInterface $api,
     ...
     *
     */
    public function getVerificationsIdentityVideos(Request $request): JsonResponse
    {
        ...
        $apiResult = $this->api->getVerificationsIdentityVideos($after, $limit, $status, $workspaceId);

        if ($apiResult instanceof \OpenAPI\Server\Model\GetVerificationsIdentityVideos200Response) {
     ...
     *
     */
    public function getVerificationsIdentityVideosId(Request $request, string $identityVideoVerificationId): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
                    'identityVideoVerificationId' => $identityVideoVerificationId,
                ],
                $request->all(),
            ),
            [
        ...
        $apiResult = $this->api->getVerificationsIdentityVideosId($identityVideoVerificationId);

        if ($apiResult instanceof \OpenAPI\Server\Model\IdentityVideoFull) {
            return response()->json($this->serde->serialize($apiResult, format: 'array'), 200);
        }
     ...
     *
     */
    public function postVerificationsIdentityVideos(Request $request): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        $initiateIdentityVideo = $this->serde->deserialize($request->getContent(), from: 'json', to: \OpenAPI\Server\Model\InitiateIdentityVideo::class);


        $apiResult = $this->api->postVerificationsIdentityVideos($initiateIdentityVideo);

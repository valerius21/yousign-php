...
use OpenAPI\Server\Api\BankAccountConnectionVerificationApiInterface;

class BankAccountConnectionVerificationController extends Controller
{
    /**
     * Constructor
     */
    public function __construct(
        private readonly BankAccountConnectionVerificationApiInterface $api,
     ...
     *
     */
    public function getVerificationsBankAccountConnections(Request $request): JsonResponse
    {
        ...
        $apiResult = $this->api->getVerificationsBankAccountConnections($after, $limit, $status, $workspaceId);

        if ($apiResult instanceof \OpenAPI\Server\Model\GetVerificationsBankAccountConnections200Response) {
     ...
     *
     */
    public function getVerificationsBankAccountConnectionsId(Request $request, string $verificationId): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        $apiResult = $this->api->getVerificationsBankAccountConnectionsId($verificationId);

        if ($apiResult instanceof \OpenAPI\Server\Model\BankAccountConnectionFull) {
            return response()->json($this->serde->serialize($apiResult, format: 'array'), 200);
        }
     ...
     *
     */
    public function postVerificationsBankAccountConnections(Request $request): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        $postVerificationsBankAccountConnectionsRequest = $this->serde->deserialize($request->getContent(), from: 'json', to: \OpenAPI\Server\Model\PostVerificationsBankAccountConnectionsRequest::class);


        $apiResult = $this->api->postVerificationsBankAccountConnections($postVerificationsBankAccountConnectionsRequest);

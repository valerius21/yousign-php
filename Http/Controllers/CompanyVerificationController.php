...
use OpenAPI\Server\Api\CompanyVerificationApiInterface;
...
class CompanyVerificationController extends Controller
{
    ...
    public function __construct(
        private readonly CompanyVerificationApiInterface $api,
     ...
     *
     */
    public function getVerificationsCompanies(Request $request): JsonResponse
    {
        ...
        $apiResult = $this->api->getVerificationsCompanies($after, $limit, $status, $workspaceId);

        if ($apiResult instanceof \OpenAPI\Server\Model\GetVerificationsCompanies200Response) {
     ...
     *
     */
    public function getVerificationsCompaniesId(Request $request, string $companyVerificationId): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
                    'companyVerificationId' => $companyVerificationId,
                ],
                $request->all(),
            ),
            [
        ...
        $apiResult = $this->api->getVerificationsCompaniesId($companyVerificationId);

        if ($apiResult instanceof \OpenAPI\Server\Model\CompanyFull) {
            return response()->json($this->serde->serialize($apiResult, format: 'array'), 200);
        }
     ...
     *
     */
    public function postVerificationsCompanies(Request $request): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        $initiateCompanyFromJson = $this->serde->deserialize($request->getContent(), from: 'json', to: \OpenAPI\Server\Model\InitiateCompanyFromJson::class);


        $apiResult = $this->api->postVerificationsCompanies($initiateCompanyFromJson);
